...
const CreateMedicalRecord = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    petId: '',
    recordType: '',
    appointmentId: '', // Yeni eklenen alan - randevu ile tıbbi kayıt ilişkisini tutmak için
  ...
  useEffect(() => {
    const loadAppointmentData = async (appointmentId) => {
      try {
        ...
        setError('Randevu bilgileri yüklenirken bir hata oluştu.');      } finally {
        setLoading(false);
      }
    };
  ...
  // Pet ID'den hayvanı ve müşteriyi yükleme için ayrı useEffect
  useEffect(() => {
    if (petIdFromUrl && customers.length > 0) {
      handlePetFromUrl(petIdFromUrl);
    }
  }, [customers, petIdFromUrl, handlePetFromUrl]);
